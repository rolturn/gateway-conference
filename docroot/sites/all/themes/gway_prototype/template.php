...
/**
 * @file
 * Contains theme override functions and preprocess functions for the gway_prototype theme.
 */
 ...
 */
function gway_prototype_html_head_alter(&$head_elements) {
...
/**
 * CSS Alter excludes css files 
 */

function gway_prototype_css_alter(&$css) {
  $exclude = array(
...
function gway_prototype_preprocess_search_block_form(&$vars) {
...
function gway_prototype_form_alter(&$form, &$form_state, $form_id) {
	//dsm($form);
 ...
 */
function gway_prototype_preprocess_html(&$vars) {
  if (module_exists('rdf')) {
    ...
    $vars['rdf']->profile = '';
	  ...
	  drupal_add_css(drupal_get_path('theme', 'gway_prototype') .'/css/fix-ie.css', array('weight' => CSS_THEME, 'browsers' => array('IE' => 'lt IE 9', '!IE' => FALSE), 'preprocess' => FALSE));
}
 ...
 */
function gway_prototype_breadcrumb($vars) {
...
function gway_prototype_menu_tree($variables) {
...
function gway_prototype_item_list($variables) {
